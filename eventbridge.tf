...
# EventBridge Rules × 5
#
# 1. kotoba-daily-story-trigger  cron UTC 09:30 (JST 18:30) → SQS   サーバー起動30分後
# 2. kotoba-rds-auto-stop        RDS-EVENT-0088 (DB起動)    → Lambda rds_stop
# 3. kotoba-ec2-auto-stop        cron UTC 15:00 (JST 00:00) → Lambda ec2_stop
# 4. kotoba-ec2-auto-start       cron UTC 09:00 (JST 18:00) → Lambda ec2_start
...
resource "aws_cloudwatch_event_rule" "daily_story_trigger" {
  ...
  schedule_expression = "cron(30 9 * * ? *)"  # JST 18:30 — サーバー起動(18:00)の30分後

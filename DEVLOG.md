...
- `eventbridge.tf`: EC2 起動を JST 09:00→18:00、停止を JST 03:00→00:00 に変更
  - daily_story トリガーを JST 00:00→18:30 に変更 (サーバー起動30分後、稼働時間外だと webhook 失敗するため)
- `budgets.tf` 新規追加: AWS Budgets で月$33 の 50%/80% アラート (CloudWatch 請求アラートは us-east-1 専用のため Budgets を採用。クレジット除外で実消費量ベースのアラート)
